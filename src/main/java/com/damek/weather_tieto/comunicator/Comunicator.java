...
public class Comunicator implements ComunicatorInterface {
	...
	// private void copyBeanProperties(final Object source, final Object target,
	...
	// src.getPropertyValue(properties.get(propertyName)));
	// }
	//
	// }
	...
	private Weather getWeather(Location location) {
		...
		Weather w = weaterDao.findByLocation(location.getLocation());
		if (page != null && page.getCurrent_observation()!= null ) {
			if (w == null) {
				w = new Weather();
			}
			// HashMap<String, String> map = new HashMap<String, String>();
			// map.put("location", "curent_observation.display_location.city");
			//
			...
			w.setWind_speed_m_per_s(page.getCurrent_observation().getWind_kph() / 3.6);
			w.setRelative_humidity(page.getCurrent_observation()
					.getRelative_humidity());
			w.setWind_speed_m_per_s(page.getCurrent_observation().getWind_mps());
			w.setWind_direction(page.getCurrent_observation().getWind_dir());
			w.setWind_description(page.getCurrent_observation()
					.getWindchill_string());
			w.setWeather_description(page.getCurrent_observation().getWeather());
